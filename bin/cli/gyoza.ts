...
try {
	await Bun.$`bun run --define=Bun.origin:"${process.env.origin}" --config=${process.env.bunfig ?? "./bunfig.toml"} --port=${process.env.port ?? "3000"} --watch ./.gyoza/dev.server.ts`.env(env).cwd(process.cwd());
} catch (error) {
	process.exit(1);
}
